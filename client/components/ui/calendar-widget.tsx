...
export function CalendarWidget({
  ...
  const renderCalendar = () => {
    ...
    for (let i = firstDay - 1; i >= 0; i--) {
      ...
      days.push(
        <div key={`prev-${day}`} className="flex items-center justify-center w-8 h-8 text-gray-400 text-xs">
          {day}
        </div>
      );
    }
    ...
    for (let day = 1; day <= remainingCells; day++) {
      days.push(
        <div key={`next-${day}`} className="flex items-center justify-center w-8 h-8 text-gray-400 text-xs">
          {day}
        </div>
      );
    }
